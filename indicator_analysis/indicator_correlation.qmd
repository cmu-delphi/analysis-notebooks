---
title: "Indicator Comparison: `r c(params$candidate_name, params$candidate_indicator, 'Candidate')[1]` vs `r c(params$guiding_name, params$guiding_indicator, 'Guiding')[1]`"
author: "Delphi Group"
date: today
format:
  html:
...
params$guiding_source    <- flatten_param(params$guiding_source)
params$guiding_indicator <- flatten_param(params$guiding_indicator)
params$guiding_name      <- flatten_param(params$guiding_name)
params$candidate_source  <- flatten_param(params$candidate_source)
params$candidate_indicator <- flatten_param(params$candidate_indicator)
params$candidate_name    <- flatten_param(params$candidate_name)
params$geo_type          <- flatten_param(params$geo_type)
params$time_type         <- flatten_param(params$time_type)
params$guiding_csv       <- flatten_param(params$guiding_csv)
params$candidate_csv     <- flatten_param(params$candidate_csv)
...
params$guiding_csv   <- resolve_path(params$guiding_csv)
params$candidate_csv <- resolve_path(params$candidate_csv)

if (!is.null(params$guiding_csv) && nzchar(params$guiding_csv)) {
    if (!file.exists(params$guiding_csv)) {
        stop(sprintf("Local guiding CSV file not found at path: %s", params$guiding_csv))
    }
}
if (!is.null(params$candidate_csv) && nzchar(params$candidate_csv)) {
    if (!file.exists(params$candidate_csv)) {
        stop(sprintf("Local candidate CSV file not found at path: %s", params$candidate_csv))
    }
}

# When using CSV route, source/indicator labels default to the CSV filename
if (!is.null(params$guiding_csv) &&
    (is.null(params$guiding_source) || !nzchar(params$guiding_source)))    params$guiding_source <- tools::file_path_sans_ext(basename(params$guiding_csv))
if (!is.null(params$guiding_csv) &&
    (is.null(params$guiding_indicator) || !nzchar(params$guiding_indicator))) params$guiding_indicator <- params$guiding_source
if (!is.null(params$candidate_csv) &&
    (is.null(params$candidate_source) || !nzchar(params$candidate_source)))    params$candidate_source <- tools::file_path_sans_ext(basename(params$candidate_csv))
if (!is.null(params$candidate_csv) &&
    (is.null(params$candidate_indicator) || !nzchar(params$candidate_indicator))) params$candidate_indicator <- params$candidate_source
# Name defaults
if (is.null(params$guiding_name) || !nzchar(params$guiding_name)) params$guiding_name <- params$guiding_indicator
if (is.null(params$candidate_name) || !nzchar(params$candidate_name)) params$candidate_name <- params$candidate_indicator
...
if (is.null(params$geo_type) || !nzchar(params$geo_type)) {
    if (!is.null(params$guiding_csv) && file.exists(params$guiding_csv)) {
        tryCatch({
            sample_df <- readr::read_csv(params$guiding_csv, n_max = 5, show_col_types = FALSE) %>%
                epiprocess::as_epi_df()
            params$geo_type <- attributes(sample_df)$metadata$geo_type
        }, error = function(e) {
            params$geo_type <- "state"
        })
    } else if (!is.null(params$candidate_csv) && file.exists(params$candidate_csv)) {
        tryCatch({
            sample_df <- readr::read_csv(params$candidate_csv, n_max = 5, show_col_types = FALSE) %>%
                epiprocess::as_epi_df()
            params$geo_type <- attributes(sample_df)$metadata$geo_type
        }, error = function(e) {
            params$geo_type <- "state"
...
# Invalidate knitr cache when params change (commented out to prevent htmlwidgets options error)
...
caption_str <- if (is.null(params$geo_type) || !nzchar(params$geo_type)) {
    sprintf("Period: %s to %s", params$start_day, params$end_day)
} else {
    sprintf("Geo-level: %s | Period: %s to %s", params$geo_type, params$start_day, params$end_day)
}

DT::datatable(config_tbl, caption = caption_str, rownames = FALSE, options = list(scrollX = TRUE, dom = "t"))
```
...
if (!is.null(params$guiding_csv)) {
    df_guiding <- read.csv(params$guiding_csv, colClasses = c(geo_value = "character")) %>%
...
if (!is.null(params$candidate_csv)) {
    df_candidate_raw <- read.csv(params$candidate_csv, colClasses = c(geo_value = "character")) %>%
        mutate(time_value = as.Date(time_value)) %>%
        filter(time_value >= as.Date(params$start_day) & time_value <= as.Date(params$end_day))
