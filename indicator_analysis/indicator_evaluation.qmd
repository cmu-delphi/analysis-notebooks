...
pacman::p_load(
    ...
    knitr, here, MMWRweek, glue, DT, withr
)
...
geo_to_name <- function(geos, geo_type = params$geo_type) {
    if (geo_type == "state") {
        names <- covidcast::abbr_to_name(geos)
    } else if (geo_type == "county") {
        names <- covidcast::county_fips_to_name(geos)
    } else if (geo_type == "msa") {
        names <- covidcast::cbsa_to_name(geos)
    } else {
        return(geos)
    }
...
if (is.null(candidate_archive) && is.null(params$candidate_csv)) {
    ...
    archive_start <- as.character(max(
        ...
        as.Date(params$end_day) - lubridate::weeks(31)
    ))
...
cat("We calculate the Coefficient of Variation (CV) for each location to compare indicator stability across geographic areas.\n\n")
...
is_pct_indicator <- grepl("%|pct", params$candidate_name, ignore.case = TRUE)
...
cat("Two additional panels are included below the correlation plot to support interpretation. The middle panel shows smoothed quantile trends for both indicators. The bottom panel shows the number of locations where both indicators were available on each date.\n\n")
```
...
cat("This histogram shows the distribution of the Spearman correlation computed within each geographic location over the full time period. The dashed line marks the median.\n\n")
```
...
cat(sprintf("To explore lead and lag relationships, we shift the candidate indicator by various intervals (%s %s). Specifically, a positive lag (e.g., +7 days) compares the candidate indicator from 7 days ago against the guiding indicator today. This allows us to see if the candidate acts as a leading indicator.\n\n", paste(lag_values, collapse = ", "), time_unit))
