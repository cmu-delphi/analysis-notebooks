...
params:
  ...
  # "Quick revision" threshold in days (used in revision_analysis() print).
  quick_revision: 7
...
if (!exists("params")) {
    params <- list(
        ...
        input                 = "notebooks/revision_analysis/data/df_raw.csv",
        geo_type              = "state",
        time_type             = "day",
...
if (!is.null(params$input)) {
    path <- here::here(params$input)

    if (fs::is_dir(path)) {
        cli::cli_inform("Reading data from directory: {.path {params$input}}")
        ...
        cli::cli_inform("Reading Parquet file: {.path {params$input}}")
        raw <- arrow::read_parquet(path)
    } else {
        cli::cli_inform("Reading CSV file: {.path {params$input}}")
        raw <- read_csv(path)
    }
    ...
    # write_csv(df_raw, here::here(params$input))

    if (nrow(df_raw) == 0 || !"issue" %in% colnames(df_raw)) {
        cli::cli_abort(
...
rev_result %>%
    print(quick_revision = params$quick_revision)
```
...
stability_combined %>%
    ...
    ggplot(aes(x = time_bucket, y = lag_stable, color = threshold)) +
    stat_summary(
    ...
    labs(
        ...
        subtitle = sprintf(
            ...
            Dots indicate the median, while lines show the IQR.",
            agg_unit
        ),
